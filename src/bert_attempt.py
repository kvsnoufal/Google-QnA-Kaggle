...
def create_model(catcols,numcols):
    ...
    model = tf.keras.models.Model(inputs=[t_id, t_mask, t_atn, cat1_input, cat2_input, cat3_input, numeric_input1, numeric_input2, numeric_input3], outputs=y)
...
for train_index, test_index in kf.split(train):
    ...
    X_tr=compute_input_arays(df=train.loc[train_index,text_feats], columns=text_feats, tokenizer=tokenizer, max_sequence_length=max_len)
    ...
    X_v=compute_input_arays(df=train.loc[test_index,text_feats], columns=text_feats, tokenizer=tokenizer, max_sequence_length=max_len)
    ...
    X_test=compute_input_arays(df=test.loc[:,text_feats], columns=text_feats, tokenizer=tokenizer, max_sequence_length=max_len)
